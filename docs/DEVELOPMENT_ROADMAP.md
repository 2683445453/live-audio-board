...
└─ settings.json 直播/监听输出设备与全局热键偏好
```
...
软件提供独立的“直播输出”和“主播监听”WASAPI 总线，每条音效可选择同时输出、仅直播
或仅监听；在线试听固定走监听总线。两个总线指向同一物理设备时会自动只播放一次，避免
重复叠音。需要严格隔离时，将直播输出设为 VB-CABLE 或 VoiceMeeter 等虚拟设备，并让
OBS 捕获该设备。OBS“应用程序音频捕获”可能同时采集本进程的监听总线，不应用于严格
隔离场景；自研虚拟声卡驱动不进入 MVP。
...
分析、建议增益的可选应用、单曲软峰值保护、播放冷却时间、最终主混音总线限幅与
实时电平反馈、可取消的资料库批量响度分析、文件缺失检测与安全重新定位，以及监听/直播
双总线和逐音效播放模式也已落地。下一阶段优先完善设备热插拔与输出故障恢复，并执行
长时间多路播放压力测试。
...
- [x] 直播/监听双总线、逐音效播放模式、同设备去重与在线试听仅监听。
...
- [x] 待分析音频批量响度分析、逐文件保存、失败隔离、进度与取消。
- [x] 文件缺失标记、托管副本自动找回、哈希校验重新定位与旧记录安全重置。
