...
- 设备热插拔与输出故障恢复、崩溃日志与长时间播放压力测试。（热插拔、恢复和自动化压力测试已完成）
...
双总线和逐音效播放模式也已落地。设备热插拔、失效设备自动回退、默认设备切换响应以及
千次双总线生命周期/32 路并发上限测试，以及带轮换保留的 UI/后台任务/进程崩溃日志已经
完成。资料库文件夹递归导入、文件/文件夹拖放、按首级子目录自动分类、音效卡拖放排序、
拖入分类与每页 8 条的宫格分页也已落地。默认麦克风/系统回环录音、最长时长、实时电平、
48 kHz 双声道 WAV 转换、首尾静音裁剪与自动入库也已完成。下一阶段进入音频片段渲染、
另存新音效与基础编辑导出工作流。
...
- [x] 输出设备热插拔、默认设备切换、失效自动回退与双总线生命周期压力测试。
- [x] 启动、UI、后台任务与进程级异常日志（本地保留最近 20 份）。
- [x] 文件夹递归导入、文件/文件夹拖放、重解析点防循环与子目录自动分类。
- [x] 音效卡拖放排序、拖入分类、SQLite 顺序持久化与筛选感知宫格分页。
- [x] 默认麦克风/系统回环录音、最长时长、实时电平、静音裁剪与 48 kHz WAV 自动入库。
